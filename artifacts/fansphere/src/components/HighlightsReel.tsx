...
function VideoModal({ videoId, title, onClose }: { videoId: string; title: string; onClose: () => void }) {
  return (
    ...
    >
      ...
      <div
        className="w-full max-w-sm aspect-video px-2"
...
export function HighlightsReel() {
  ...
  const title = (h: typeof highlights[0]) =>
        ...
        <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide -mx-4 px-4">
          {highlights.map((h) => {
            ...
            return (
              <div
                key={h.id}
                className="flex-shrink-0 w-44 rounded-xl overflow-hidden border border-[#E2E8F0] bg-white shadow-sm active:scale-[0.97] transition-transform cursor-pointer"
                onClick={() =>
                  hasEmbed
              ...
              >
                {/* Thumbnail */}
                <div className="relative h-24 bg-gradient-to-br from-[#1E3A5F] to-[#2563EB] overflow-hidden">
                  {h.thumbnail ? (
                    <img
                      src={h.thumbnail}
                      alt={title(h)}
                  ...
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                    <div className="w-9 h-9 rounded-full bg-white/90 flex items-center justify-center">
                      {hasEmbed ? (
                        <Play className="w-4 h-4 text-[#DC2626] fill-[#DC2626] ml-0.5" />
                      ) : (
