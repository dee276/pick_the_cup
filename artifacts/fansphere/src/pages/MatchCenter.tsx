...
export default function MatchCenter() {
  ...
  return (
    <div className="flex flex-col min-h-full" data-testid="match-center">
      {/* Header */}
      <div className="bg-gradient-to-br from-primary to-blue-800 text-white px-4 pt-10 pb-8">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/">
            <button className="p-1.5 rounded-full bg-white/10 hover:bg-white/20 transition-colors" data-testid="button-back">
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col items-center gap-2 flex-1" data-testid="team-a-info">
            ...
            <span className="font-bold text-base">{match.teamA.code}</span>
          </div>

          <div className="flex flex-col items-center gap-1 flex-1">
            {isLive || isFinished ? (
              <span className="text-5xl font-black tracking-tight" data-testid="score-display">
            ...
            <span className="text-xs text-white/60 mt-1">{match.group} · {match.stadium}</span>
          </div>
          ...
          <div className="flex flex-col items-center gap-2 flex-1" data-testid="team-b-info">
            ...
            <span className="font-bold text-base">{match.teamB.code}</span>
          </div>
        </div>
      </div>

      <div className="flex-1 p-4 space-y-5 bg-slate-50">
        {/* User prediction callout */}
        {userPrediction && (
          <div className="rounded-xl bg-[#EFF6FF] border border-[#BFDBFE] p-4" data-testid="prediction-callout">
              ...
              {events.map((event, i) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-[#64748B]">{event.team} · {eventTypeLabel[event.type] ?? event.type}</p>
                  </div>
                </div>
              ))}
            </div>
